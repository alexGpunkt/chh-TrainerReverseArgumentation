...
function currentStage(){ return state.data.stages[state.stageIndex]; }
function currentTask(){ return currentStage().tasks[state.taskIndex]; }
...
function renderTask(t){
  ...
  if(t.type === "choice"){
    t.options.forEach((opt,i)=>{
      ...
      label.innerHTML = `<input type="radio" name="choice" value="${i}"><span>${opt}</span>`;
  ...
  if(t.type === "match"){
    t.pairs.forEach((pair,i)=>{
      ...
      const shuffled = [...t.pairs.map(p=>p[1])].sort(()=>Math.random()-.5);
      row.innerHTML = `<strong>${pair[0]}</strong>`;
      const sel = document.createElement("select");
      sel.dataset.answer = pair[1];
      sel.innerHTML = `<option value="">Passende Frage wählen…</option>` + shuffled.map(x=>`<option>${x}</option>`).join("");
      row.appendChild(sel);
  ...
  if(t.type === "rewrite"){
    ...
    info.textContent = "Tipp: Nutze Wörter wie „vielleicht“, „könnte“, „gleichzeitig“, „unter der Bedingung“, „ein Gegenargument wäre …“.";
...
function check(){
  ...
  if(t.type === "choice"){
    ...
    ok = checked && Number(checked.value) === t.answer;
  }

  if(t.type === "cloze"){
    const value = norm($("#clozeInput").value);
    ok = (t.accepted || [t.answer]).some(a => norm(a) === value);
  }
  ...
  if(t.type === "rewrite"){
    const value = norm($("#rewriteInput").value);
    ok = value.length >= 25 && t.mustContainAny.some(word => value.includes(norm(word)));
  }
  ...
  if(ok){
    ...
    fb.textContent = "Noch nicht ganz. Lies den Hinweis im Menü oder formuliere vorsichtiger und fairer.";
...
function next(){
  ...
  if(state.taskIndex < st.tasks.length - 1){
    ...
    $("#feedback").textContent = "Alle Etappen geschafft. Du kannst den Fortschritt zurücksetzen oder die Aufgaben in tasks.json erweitern.";
...
function renderProgress(){
  ...
  const percent = allTasks.length ? Math.round((solved / allTasks.length) * 100) : 0;
  $("#trackFill").style.width = `${percent}%`;
  $("#walker").style.left = `${percent}%`;
  $("#progressText").textContent = `${solved} von ${allTasks.length} Aufgaben gelöst (${percent}%).`;
  $("#badgeList").innerHTML = state.badges.length
    ? state.badges.map(b=>`<span class="badge">${b}</span>`).join("")
    : "<span>Noch kein Abzeichen.</span>";
...
function renderQr(text){
  ...
  // Hinweis: Dies ist ein stabiler Platzhalter-Code. Für echte QR-Codes kann später eine QR-Library ergänzt werden.
}
